...
void WB() {
	...
	for (i = 0; i < 16384; i++) {   // initialization histogram 
		r_hist[i]=0;
		g_hist[i]=0;
		b_hist[i]=0;
	}
...
//			  b_hist[(pic[i][j+1]/2)]++;  // E
	...
	if (counterCr1 + counterCr2 + counterCb1 + counterCb2 == 0) {
      ...
      /*----White Balance Adjustment----*/
			...
			if (B_hist_avg + 192 >= G_hist_avg && B_hist_avg > R_hist_avg){  // bluish
                R_factor = R_scale;
			    G_factor = G_scale;
                B_factor = B_GWA;
				printf("bluish\n");
...
*/          if (!(R_factor == 1 && G_factor == 1 && B_factor == 1)){
		  ...
		  printf("not do the white balance... \n");
	  }
 ...
 // 	 
	...
	for (i = 0; i < 2460; i++) {
		for (j = 0; j < 3360; j++) {
			...
			g[i][j] = (double)((-0.1875) * temp_r + (1.203125) * temp_g + (-0.046875) * temp_b);
			...
			b[i][j] = (double)((-0.078125) * temp_r + (-0.296875) * temp_g + (1.359375) * temp_b);
